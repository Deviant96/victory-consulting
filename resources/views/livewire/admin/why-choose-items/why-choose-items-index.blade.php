{{-- Why Choose Items Index Component --}}
<div>
    {{-- Header --}}
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Why Choose Us Items</h1>
            <p class="text-sm text-slate-600 mt-1">Manage the reasons why customers choose your business</p>
        </div>
    ...
    <div class="admin-card overflow-hidden">
        @if($items->count() > 0)
            <div class="divide-y divide-slate-200">
                @foreach($items as $item)
                    <div wire:key="item-{{ $item->id }}" class="p-5 hover:bg-slate-50 transition">
                        <div class="flex items-start gap-4">
                            {{-- Order Controls --}}
                            <div class="flex flex-col gap-1 shrink-0">
                                <button 
                                    wire:click="updateOrder({{ $item->id }}, 'up')"
                                ...
                                <span class="text-xs text-slate-500 text-center font-medium">{{ $item->order }}</span>
                                <button 
                                    wire:click="updateOrder({{ $item->id }}, 'down')"
                                    class="p-1 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition"
                                    title="Move down">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-4 mb-2">
                                    ...
                                    <div class="flex items-center gap-2 shrink-0">
                                        {{-- Status Badge --}}
                                        <span class="px-2 py-1 text-xs font-medium rounded-full {{ $item->is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600' }}">
                                ...
                                {{-- Icon Class Display --}}
                                <p class="text-xs text-slate-500 mb-3 font-mono">{{ $item->icon }}</p>

                                {{-- Actions --}}
                                <div class="flex items-center gap-2">
                                    <button 
                                        wire:click="toggleActive({{ $item->id }})"
                                        class="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium rounded-lg transition {{ $item->is_active ? 'bg-slate-100 text-slate-700 hover:bg-slate-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }}">
                                        @if($item->is_active)
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('admin.why-choose-items.edit', $item->id) }}" 
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <h3 class="text-lg font-semibold text-slate-900 mb-1">No items found</h3>
                <p class="text-slate-600 mb-4">
                    @if($search || $statusFilter !== '')
                        No items match your filters. Try adjusting your search criteria.
                    @else
                        Get started by creating your first "Why Choose Us" item.
                    @endif
                </p>
                @if($search || $statusFilter !== '')
                    <button 
