...
<div class="bg-white rounded-xl shadow p-6">
    ...
    <form action="{{ route('admin.faqs.update', $faq) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="question" id="question" value="{{ old('question', $faq->translate('question', config('app.fallback_locale'))) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 @error('question') border-red-500 @enderror">
            @error('question')
        ...
        <div class="mb-6">
            ...
            <textarea name="answer" id="answer" rows="6" required class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 @error('answer') border-red-500 @enderror">{{ old('answer', $faq->translate('answer', config('app.fallback_locale'))) }}</textarea>
            @error('answer')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
