    ...
    @else
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div>
                    <h2 class="text-base font-semibold text-gray-900">Inbox</h2>
            ...
            @forelse($messages as $message)
                @php
                    $isSeen = $message->getFlags()->contains('seen') ?? false;
                    $uid = $message->getUid();
                    $from = $message->getFrom()->first();
                    $subject = $message->getSubject()->first() ?? '(no subject)';
                    $date = $message->getDate()->first();
                @endphp
                <a href="{{ route('admin.email.show', ['uid' => $uid, 'folder' => 'inbox']) }}"
                   class="flex items-start gap-4 px-6 py-4 border-b border-gray-50 hover:bg-blue-50/30 transition group">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-sm {{ $isSeen ? 'text-gray-600' : 'font-semibold text-gray-900' }} truncate">
                                {{ $from ? $from->mail : 'Unknown sender' }}
                            </span>
                            <span class="text-xs text-gray-400 flex-shrink-0">
                                {{ $date ? \Carbon\Carbon::parse($date)->diffForHumans() : '' }}
                            </span>
                        </div>
                        <p class="text-sm {{ $isSeen ? 'text-gray-500' : 'text-gray-800 font-medium' }} truncate mt-0.5">{{ $subject }}</p>
                    </div>
