...
<div class="bg-white rounded-xl shadow p-6">
    <h1 class="text-3xl font-bold text-gray-900 mb-6">Add Mission</h1>

    <form action="{{ route('admin.missions.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-6">
            ...
            <textarea name="content" id="content" rows="6" required class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500">{{ old('content') }}</textarea>
            @error('content')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
            <p class="text-gray-500 text-sm mt-1">Recommended: Image of a man offering handshake positioned to the right</p>
        </div>
