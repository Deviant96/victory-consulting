...
@section('page-description', 'Find content by keywords, category, or status with streamlined controls.')
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <form method="GET" class="flex flex-col sm:flex-row gap-3">
                ...
                <input type="text" name="category" value="{{ $category ?? '' }}" placeholder="Category"
                       class="px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition bg-white/80">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.articles.index') }}" class="px-4 py-2 border border-gray-200 rounded-xl text-gray-700 hover:bg-gray-50 transition">Reset</a>
                </div>
            </form>
            <a href="{{ route('admin.articles.create') }}" class="bg-gradient-to-r from-blue-600 to-indigo-600 hover:shadow-md text-white px-6 py-2 rounded-xl font-semibold transition transform hover:-translate-y-0.5">
                Add Article
            </a>
        </div>
    </div>
                    ...
                    <tr class="hover:bg-blue-50/40">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <img src="{{ asset('storage/' . $post->featured_image) }}" alt="{{ $post->title }}" class="h-10 w-16 object-cover rounded-lg mr-3">
                                @endif
                                <div>
                                    <div class="text-sm font-semibold text-gray-900">{{ Str::limit($post->title, 60) }}</div>
                                    <div class="text-xs text-gray-500">{{ Str::limit($post->excerpt, 80) }}</div>
                                </div>
                            </div>
                        </td>
