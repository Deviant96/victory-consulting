...
<div class="space-y-8">
    <section class="grid gap-6 xl:grid-cols-3">
        <div class="xl:col-span-2 bg-gradient-to-r from-indigo-600 via-blue-600 to-sky-500 text-white rounded-3xl shadow-lg p-6 sm:p-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                <div class="space-y-2">
                    <p class="text-xs uppercase tracking-widest text-blue-100">Admin overview</p>
                    <h1 class="text-2xl sm:text-3xl font-semibold">Welcome back</h1>
                    <p class="text-sm text-blue-100 max-w-xl">Monitor performance, curate fresh content, and keep communication flowing smoothly—all from one streamlined dashboard.</p>
                </div>
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl px-4 py-3 w-full sm:w-64">
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-blue-100">Pending bookings</span>
                    ...
                    <div class="mt-3 h-2 rounded-full bg-white/20 overflow-hidden">
                        <div class="h-full bg-white/80 rounded-full" style="width: {{ $totalBookings > 0 ? min(100, ($pendingBookings / max($totalBookings, 1)) * 100) : 12 }}%"></div>
                    </div>
                    <p class="mt-3 text-xs text-blue-100">Stay responsive to requests and keep prospects moving forward.</p>
                </div>
            </div>

            <div class="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div class="bg-white/10 border border-white/10 rounded-2xl p-3">
                    <p class="text-xs text-blue-100">Services live</p>
                    <p class="text-xl font-semibold">{{ number_format($stats['services'] ?? 0) }}</p>
                </div>
                <div class="bg-white/10 border border-white/10 rounded-2xl p-3">
                    <p class="text-xs text-blue-100">Published articles</p>
                    <p class="text-xl font-semibold">{{ number_format($stats['articles'] ?? 0) }}</p>
                </div>
                <div class="bg-white/10 border border-white/10 rounded-2xl p-3">
                    <p class="text-xs text-blue-100">FAQs maintained</p>
                    <p class="text-xl font-semibold">{{ number_format($stats['faqs'] ?? 0) }}</p>
                </div>
                <div class="bg-white/10 border border-white/10 rounded-2xl p-3">
                    <p class="text-xs text-blue-100">Bookings logged</p>
                    <p class="text-xl font-semibold">{{ number_format($stats['bookings'] ?? 0) }}</p>
                </div>
            </div>
        </div>

        <div class="bg-white border border-gray-200 rounded-3xl shadow-sm p-6 flex flex-col gap-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs uppercase tracking-wider text-gray-500">Today</p>
            ...
            <div class="rounded-2xl bg-gray-50 border border-dashed border-gray-200 px-4 py-3">
    ...
    <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        @foreach ($statCards as $card)
            <a href="{{ $card['link'] }}" class="group relative overflow-hidden bg-white border border-gray-200 rounded-2xl shadow-sm p-5 transition transform hover:-translate-y-1 hover:shadow-md">
    ...
    <section class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-2 bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
            <div class="flex items-center justify-between gap-3 flex-wrap">
                <div>
                    <p class="text-xs uppercase tracking-wider text-gray-500">Bookings</p>
        ...
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 flex flex-col gap-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs uppercase tracking-wider text-gray-500">Content</p>
    ...
    <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs uppercase tracking-wider text-gray-500">Quick actions</p>
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach ($quickActions as $action)
                    <a href="{{ $action['href'] }}" class="group border border-gray-100 rounded-2xl p-4 hover:border-{{ $action['color'] }}-200 hover:bg-{{ $action['color'] }}-50/40 transition flex flex-col gap-2">
                        ...
                        <span class="text-xs font-semibold text-{{ $action['color'] }}-700">Start →</span>
                    </a>
                @endforeach
            </div>
        </div>
