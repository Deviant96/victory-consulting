...
<div class="space-y-6">
    ...
    <form action="{{ route('admin.pages.about.update') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <!-- Founder's Wisdom Card -->
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    <h2 class="text-lg font-medium text-gray-900">Founder's Wisdom</h2>
                    <span class="text-xs text-gray-500 uppercase tracking-wide font-semibold">Core Values</span>
                </div>
                <div class="p-6 space-y-8">
                    
                    <!-- Principles Intro -->
                    <div class="bg-orange-50 rounded-xl p-4 border border-orange-100 space-y-4">
                        <x-admin.settings-field-with-translation
                            name="about[principles_heading]"
                            label="Section Heading"
                            value="Founder's Principles"
                            :settings="$settings"
                            :languages="$languages"
                        />
                        <x-admin.settings-field-with-translation
                            name="about[principles_subheading]"
                            label="Section Subheading"
                            type="textarea"
                            rows="2"
                            value="The values that shape our consulting quality."
                            :settings="$settings"
                            :languages="$languages"
                        />
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        <!-- Wisdom 1 -->
                        <div class="space-y-4 p-4 border border-gray-100 rounded-xl hover:border-blue-200 transition">
                            ...
                            <x-admin.settings-field-with-translation
                                name="about[wisdom1_title]"
                                label="Title"
                                value="Melayani Sepenuh Hati"
                                :settings="$settings"
                                :languages="$languages"
                            />
                            <x-admin.settings-field-with-translation
                                name="about[wisdom1_description]"
                                label="Description"
                                type="textarea"
                                rows="2"
                                value="Klien dari Seluruh Indonesia"
                                :settings="$settings"
                                :languages="$languages"
                            />
                            ...
                            <!-- Wisdom 1 Image -->
                             <div class="pt-2">
                                ...
                                @if(optional($settings->get('about.wisdom1_image'))->value)
                                    <img src="{{ asset('storage/' . $settings->get('about.wisdom1_image')->value) }}" alt="Wisdom 1" class="w-16 h-16 object-contain bg-gray-50 rounded-lg border border-gray-200 mb-2">
                                @endif

                                <!-- Preview Container -->
                                <div id="wisdom1-preview-container" class="hidden relative group rounded-lg overflow-hidden border-2 border-blue-500 shadow-md w-16 h-16 mb-2">
                                    <img id="wisdom1-preview" src="" alt="Preview" class="w-full h-full object-contain bg-white">
                                    <button type="button" id="clear-wisdom1-btn" class="absolute top-0.5 right-0.5 bg-red-500 hover:bg-red-600 text-white w-4 h-4 flex items-center justify-center rounded-full shadow transition text-xs leading-none">
                                        &times;
                                    </button>
                                </div>

                                <input type="file" name="wisdom1_image" id="wisdom1_image" class="block w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 transition"/>
                            </div>
                        </div>

                        <!-- Wisdom 2 -->
                        <div class="space-y-4 p-4 border border-gray-100 rounded-xl hover:border-green-200 transition">
                            <h3 class="font-semibold text-gray-900 flex items-center text-sm">
                                <span class="bg-green-100 text-green-700 w-6 h-6 rounded-full flex items-center justify-center text-xs mr-2">2</span>
                                Wisdom Card 2 (Growth)
                            </h3>
                            <x-admin.settings-field-with-translation
                                name="about[wisdom2_title]"
                                label="Title"
                                value="Membantu Bisnis Tumbuh"
                                :settings="$settings"
                                :languages="$languages"
                            />
                            <x-admin.settings-field-with-translation
                                name="about[wisdom2_description]"
                                label="Description"
                                type="textarea"
                                rows="2"
                                value="dan UMKM Berkembang"
                                :settings="$settings"
                                :languages="$languages"
                            />
                             ...
                             <div class="pt-2">
                                ...
                                @if(optional($settings->get('about.wisdom2_image'))->value)
                                    <img src="{{ asset('storage/' . $settings->get('about.wisdom2_image')->value) }}" alt="Wisdom 2" class="w-16 h-16 object-contain bg-gray-50 rounded-lg border border-gray-200 mb-2">
                                @endif

                                <!-- Preview Container -->
                                <div id="wisdom2-preview-container" class="hidden relative group rounded-lg overflow-hidden border-2 border-blue-500 shadow-md w-16 h-16 mb-2">
                                    <img id="wisdom2-preview" src="" alt="Preview" class="w-full h-full object-contain bg-white">
                                    <button type="button" id="clear-wisdom2-btn" class="absolute top-0.5 right-0.5 bg-red-500 hover:bg-red-600 text-white w-4 h-4 flex items-center justify-center rounded-full shadow transition text-xs leading-none">
                                        &times;
                                    </button>
                                </div>

                                <input type="file" name="wisdom2_image" id="wisdom2_image" class="block w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 transition"/>
                            </div>
                        </div>
                    </div>
                </div>
