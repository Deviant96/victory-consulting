...
@section('title', 'Create Sub-Solution')
@section('page-title', 'Create Sub-Solution')
@section('page-description', 'Add a new sub-industry to a business solution')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Create Sub-Solution</h1>
    <p class="text-gray-600 mt-2">Add a new sub-industry to a business solution</p>
</div>
...
<div class="bg-white rounded-xl shadow p-6">
    <form action="{{ route('admin.sub-solutions.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-6">
            ...
            <select name="business_solution_id" id="business_solution_id" required class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 @error('business_solution_id') border-red-500 @enderror">
                ...
                <option value="{{ $solution->id }}" {{ old('business_solution_id') == $solution->id ? 'selected' : '' }}>
                    {{ $solution->title }}
                </option>
                @endforeach
            </select>
        ...
        <div class="mb-6">
            ...
            <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="e.g., E-commerce Solutions" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 @error('title') border-red-500 @enderror">
            @error('title')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="number" name="order" id="order" value="{{ old('order', 0) }}" min="0" required class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 @error('order') border-red-500 @enderror">
            @error('order')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
